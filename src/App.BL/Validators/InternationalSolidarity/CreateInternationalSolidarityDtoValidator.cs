...
public class CreateInternationalSolidarityDtoValidator : AbstractValidator<CreateInternationalSolidarityDto>
{
    private static readonly string[] AllowedContentTypes =
        ["image/jpeg", "image/png", "image/gif", "image/webp"];
    private const long MaxFileSizeBytes = 5 * 1024 * 1024;

    public CreateInternationalSolidarityDtoValidator(ILanguageService languageService)
    {
        RuleFor(x => x.Link)
        ...
        RuleFor(x => x.Icon)
            .NotNull().WithMessage(ValidationMessages.IconRequired(languageService.Lang));
