...
public class CreatePublicationDtoValidator : AbstractValidator<CreatePublicationDto>
{
    ...
    private const long MaxPdfSizeBytes = 50 * 1024 * 1024;    // 10 MB

    public CreatePublicationDtoValidator(ILanguageService languageService)
    {
        RuleFor(x => x.TitleAz)
        ...
        RuleFor(x => x.PdfFile)
            .NotNull().WithMessage(ValidationMessages.PdfFileRequired(languageService.Lang));
